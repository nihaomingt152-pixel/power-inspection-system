...
async function loadMarkers() {
    ...
    const colors = { '一般': '#34c759', '严重': '#ff9f0a', '紧急': '#ff3b30' };
    const bounds = [];
        ...
        const popup = `
            <div class="map-popup">
                <div class="map-popup-head">
                    <span class="map-popup-title">${m.class_name}</span>
                    <span class="map-popup-badge" style="background:${color};">${m.severity}</span>
                </div>
                <div class="map-popup-time">${m.created_at ? new Date(m.created_at).toLocaleString() : '-'}</div>
                ${m.annotated_image_path ? `<img src="${buildPreviewUrl(m.annotated_image_path)}" alt="检测图">` : ''}
                <a class="map-popup-link" href="/orders">查看工单 →</a>
            </div>`;
    ...
    orders.forEach(o => {
        ...
        const popup = `
            <div class="map-popup">
                <div class="map-popup-head">
                    <span class="map-popup-title">📋 工单 #${o.id}</span>
                    <span class="map-popup-badge" style="background:${color};">${o.status_text}</span>
                </div>
                <div class="map-popup-subtitle">${o.title}</div>
                <div class="map-popup-row"><span>严重度</span><b>${o.severity}</b></div>
                <div class="map-popup-row"><span>检修人</span><b>${o.assignee || '未指派'}</b></div>
                <div class="map-popup-time">${o.created_at ? new Date(o.created_at).toLocaleString() : '-'}</div>
                ${o.annotated_image_path ? `<img src="${buildPreviewUrl(o.annotated_image_path)}" alt="检测图">` : ''}
                <a class="map-popup-link" href="/orders">去工单管理 →</a>
            </div>`;
