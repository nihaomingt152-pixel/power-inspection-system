...
async function showOrderDetail(id) {
    try {
        ...
        if (origImg || repairImg) {
            imgHtml = '<div class="order-images">';
            if (origImg) {
                imgHtml += `<div class="order-image-card">
                    <div class="order-image-title">🔍 原始缺陷图（YOLO标注）</div>
                    <div class="order-image-shell"><img src="${origImg}" class="compare-img" onclick="zoomImage('${origImg}')" title="点击放大"></div>
                </div>`;
            }
            if (repairImg) {
                imgHtml += `<div class="order-image-card">
                    <div class="order-image-title">🔧 修复照片（Worker提交）</div>
                    <div class="order-image-shell"><img src="${repairImg}" class="compare-img" onclick="zoomImage('${repairImg}')" title="点击放大"></div>
                </div>`;
            }
            imgHtml += '</div>';
        }
        ...
        // Worker 备注
        if (r.review_remark) {
            imgHtml += `<div class="order-remark"><span>📝</span><span>Worker 备注：${r.review_remark}</span></div>`;
        }
        ...
        if (r.ai_summary) {
            ...
            if (overall || risk) {
                aiSummaryHtml = `<section class="result-block ai-report detail-section">
                    <h6>🤖 AI 分析摘要</h6>
                    <dl class="detail-kv">
                        ${risk ? `<div><dt>风险等级</dt><dd><span class="badge bg-${riskClass}">${risk}</span></dd></div>` : ''}
                        ${overall ? `<div><dt>${vs.overall_description ? '总体描述' : '描述'}</dt><dd>${overall}</dd></div>` : ''}
                        ${vs.suggestions ? `<div><dt>建议</dt><dd>${vs.suggestions}</dd></div>` : ''}
                        ${vs.focus_points ? `<div><dt>重点关注</dt><dd>${vs.focus_points}</dd></div>` : ''}
                        ${vs.cause ? `<div><dt>成因</dt><dd>${vs.cause}</dd></div>` : ''}
                    </dl>
                </section>`;
            }
        }
            ...
            body.innerHTML = `
                <div class="order-detail-hero">
                    ...
                    <div class="order-detail-facts">
                        <div class="order-detail-fact"><span>严重程度</span><span><span class="severity-tag severity-${r.severity}">${r.severity}</span></span></div>
                        <div class="order-detail-fact"><span>创建者</span><span>${r.creator_name || '#' + r.created_by}</span></div>
                        <div class="order-detail-fact"><span>检修人</span><span>${r.assignee_name || '#' + r.assigned_to}</span></div>
                        <div class="order-detail-fact"><span>GPS</span><span>${r.gps_lat ? r.gps_lat + ', ' + r.gps_lng : '无'}</span></div>
                        <div class="order-detail-fact"><span>创建时间</span><span>${r.created_at ? new Date(r.created_at).toLocaleString() : ''}</span></div>
                        ${r.reject_reason ? `<div class="order-detail-fact"><span>驳回理由</span><span>${r.reject_reason}</span></div>` : ''}
                    </div>
                    ${r.description ? `<div class="order-detail-desc"><strong>描述:</strong> ${r.description}</div>` : ''}
                </div>
                ${imgHtml}
                ${aiSummaryHtml}
